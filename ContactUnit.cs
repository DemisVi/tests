...
public class ContactUnit
{
    ...
    protected double poolingInterval = 500D;
...
#pragma warning restore CS8618 // singleton
    ...
    public ContactUnit(string serial, double poolingInterval = 500D, Action<string>? logMethod = null)
    {
    ...
    // public static ContactUnit GetInstance(string serial, double poolingInterval = 500D, Action<string>? logMethod = null) =>
    //     _cu ??= new ContactUnit(serial, poolingInterval, logMethod);

    public ContactUnit(string serial, string portName, double poolingInterval = 500D, Action<string>? logMethod = null)
    {
        _adapter = new(serial + _cuSerialPrefix, logMethod);
        _ftSerialPort = portName;
        _tcs = new();
        this.poolingInterval = poolingInterval;
    }

    // public static ContactUnit GetInstance(string serial, string portName, double poolingInterval = 500D, Action<string>? logMethod = null) =>
    //     _cu ??= new ContactUnit(serial, poolingInterval, logMethod);

    public string PortName => _ftSerialPort;
    public string AdapterSerial => _adapter.SerialNum;
